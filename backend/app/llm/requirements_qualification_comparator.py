...
from app.models import Comparisons, JobPostings, Users


class RequirementsQualificationComparison(BaseModel):
    priority: int = Field(
        ...,
        description="In your opinion, how important is this requirement compared to the others?",
    )
    job_description_requirement: str = Field(
        ..., description="The job description requirement"
    )
    cv_qualification: str = Field(..., description="The cv qualification")
    cv_qualification_written_in_requirement_language: str = Field(
        ...,
        description="The cv qualification, re-written to use the job description requirement language style",
    )


class RequirementsQualificationComparisonList(BaseModel):
    requirements_qualification_comparisons: List[
        RequirementsQualificationComparison
    ] = Field(..., description="A list of requirements and qualifications comparisons")
...
class RequirementsQualificationsComparator:
    def __init__(
        ...
        self.output_parser = JsonOutputParser(
            pydantic_object=RequirementsQualificationComparisonList
        )
        ...
        self.template = PromptTemplate(
            ...
            partial_variables={
                ...
                "parsed_worked_experiences": self._get_user_attribute(
                    user_id=user_id, attribute="parsed_work_experiences"
                ),
                "parsed_skills": self._get_user_attribute(
                    user_id=user_id, attribute="parsed_skills"
                ),
            },
        )
    ...
    def _get_user_attribute(self, user_id: int, attribute: str) -> str:
        ...
        try:
            user_columns = [
                getattr(UsersModel, attr)
                for attr in UsersModel.__table__.columns.keys()
    ...
    def _get_job_posting(self, job_id: int):
        ...
        try:
            job_posting_columns = [
                getattr(JobPostingModel, attr)
                for attr in JobPostingModel.__table__.columns.keys()
                if attr in ["job_posting_summary"]
            ]
            job_posting = (
                session.query(*job_posting_columns)
    ...
    def _run_chain(
        self, job_posting_data: str
    ) -> RequirementsQualificationComparisonList:
    ...
    def _compare(
        self, job_id: int, replace_previous_comparison_flag: bool = False
    ) -> dict:
        self.logger.info(
            f"Comparing requirements and qualifications for user {self.user_id} and job posting {job_id}"
        )
        job_posting = self._get_job_posting(job_id=job_id)
        session = self.db.get_session()
        try:
            comparison = (
                session.query(UserJobComparisonModel)
                .filter(
                    UserJobComparisonModel.job_posting_id == job_id,
            ...
            if comparison:
                if replace_previous_comparison_flag:
                    comparison.comparison = self._run_chain(
                        job_posting_data=job_posting
                    )
                    session.commit()
                    self.logger.info(
                        f"Updated comparison for user {self.user_id} and job posting {job_id}"
                    )
                else:
                    self.logger.info(
                        f"Comparison already exists for user {self.user_id} and job posting {job_id}"
                    )
            else:
                comparison = UserJobComparisonModel(
                ...
                self.logger.info(
                    f"Added comparison for user {self.user_id} and job posting {job_id}"
                )
        except Exception as e:
    ...
    def compare_job_postings_with_user(
        self, job_ids: List[int], replace_previous_comparison_flag: bool = False
    ):
        """Parallel implementation of the compare method"""
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_job_id = {
                executor.submit(
                    self._compare, job_id, replace_previous_comparison_flag
                ): job_id
            ...
            for future in as_completed(future_to_job_id):
                ...
                try:
                    ...
                    self.logger.error(
                        f"An error occurred while comparing job posting {job_id}: {exc}"
                    )


if __name__ == "__main__":
    comparator = RequirementsQualificationsComparator(
        model_name=ModelNames.GPT3_TURBO, user_id=1, temperature=0
    )
    comparator.compare_job_postings_with_user(
        job_ids=[3872263836], replace_previous_comparison_flag=True
    )
